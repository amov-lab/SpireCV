...
void _parser_algorithm_params(std::string name_, JsonValue& f_value_, JsonValue& params_value_)
{
  bool has_params = false;
  if (f_value_.getTag() == JSON_OBJECT) {
    for (auto i : f_value_) {
      if (name_ == std::string(i->key)) {
        // std::cout << i->key << std::endl;
        params_value_ = i->value;
        has_params = true;
      }
    }
  }

  if (!has_params && params_value_.getTag() == JSON_OBJECT)
  {
    char msg[256];
    sprintf(msg, "SpireCV (106) %s parameters reading ERROR!", name_.c_str());
    throw std::runtime_error(msg);
  }
}

void _load_all_json(std::string json_fn, JsonValue& value, JsonAllocator& allocator)
{
  std::ifstream fin;
  fin.open(json_fn, std::ios::in);
  if (!fin.is_open())
  {
    throw std::runtime_error("SpireCV (104) Algorithm parameters file NOT exist!");
  }
  std::string json_str, buff;
  json_str = "";
  while (getline(fin, buff))
  {
    json_str = json_str + _trim(buff);
  }
  fin.close();
